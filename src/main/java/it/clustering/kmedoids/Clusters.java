...
public class Clusters {
	 ...
	 */
	public Clusters(ArrayList<Instance> medoids, DistanceFunction distanceFunction) {

		log.debug("Clusters constructor medoids:\n" + medoids.toString());

		this.distanceFunc = distanceFunction;

		this.clusters = new ArrayList<Cluster>(medoids.size());
		for (int i = 0; i < medoids.size(); i++) {
			clusters.add(new Cluster(medoids.get(i), distanceFunction));
		}
	}
	...
	public double assignToTheNearestCluster(ArrayList<Instance> elements) {
		...
		for (Instance element : elements) {
			...
			Cluster selectedCluster = clusters.get(0);
	...
	public ArrayList<Instance> getMedoids() {
		...
		ArrayList<Instance> new_medoids = new ArrayList<Instance>(this.clusters.size());
		for (int i = 0; i < clusters.size(); i++) {
