 ...
 *
 */
public class KMedoidsClusters extends Clusters{
	 ...
	 */
	public KMedoidsClusters(ArrayList<Instance> medoids, DistanceFunction distanceFunction) {
		super(new ArrayList<KMedoidsCluster>(medoids.size()));
		...
		for (int i = 0; i < medoids.size(); i++) {
			clusters.add(new KMedoidsCluster(medoids.get(i), distanceFunction));
		}
	}
	 ...
	 */
	public double assignToTheNearestCluster(ArrayList<Instance> elements) {
		double sum = 0;
		for (Instance element : elements) {
			// Let's scan all the clusters
			// for identify the closest medoid
			
			KMedoidsCluster selectedCluster = clusters.get(0);
			Instance closestMedoid = selectedCluster.getMedoid();
			double closestMedoidDistance = this.distanceFunc.distance(element, closestMedoid);
			
			
			for (KMedoidsCluster cluster : this.clusters) {
				
				Instance iterationMedoid = cluster.getMedoid();
	 ...
	 */
	public ArrayList<Instance> getMedoids() {
		// new medoids will be part of clustering algorithm if
		// they pass the stop criterion phase
		ArrayList<Instance> new_medoids = new ArrayList<>(this.clusters.size());
		for (int i = 0; i < clusters.size(); i++) {
